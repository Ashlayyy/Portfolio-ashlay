...
class Navigation {
	...
	settings = undefined;
	names = ['Home', 'Over Mij', 'Projecten', 'Contact'];
	classNames = ['Home', 'AboutMe', 'Projects', 'Contact'];
	data = undefined;
	homePage = undefined;
	...
	generateListItems = () => {
		for (let i = 0; i < this.names.length; i++) {
			...
			if (i < 3) {
				...
				this.homePage.RenderEngine.render(
					this.listItemAElement,
					this.buttonElement
				);
			}
			this.homePage.RenderEngine.render(
				this.listItemLiElement,
				this.listItemAElement
			);
			this.homePage.RenderEngine.render(
